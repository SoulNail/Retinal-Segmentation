...
from models.Additional.DWT.DWT import *
import pywt
...
class DWT_2D(Module):
    ...
    def get_matrix(self):
        ...
        # 初始化变换矩阵尺寸
        matrix_h = np.zeros((out_H, H + self.band_length - 2))
        matrix_g = np.zeros((out_H, H + self.band_length - 2))
        ...
        # 对 height（行）方向的 low/high filter
        matrix_h_0 = matrix_h[:, (self.band_length_half - 1): -(self.band_length_half - 1) or None]
        matrix_g_0 = matrix_g[:, (self.band_length_half - 1): -(self.band_length_half - 1) or None]

        # 对 width（列）方向 filter，用转置的方式构造列变换
        matrix_h_1 = matrix_h_0.T.copy()
        matrix_g_1 = matrix_g_0.T.copy()

        # 转为 tensor，并移动到 GPU
        if torch.cuda.is_available():
            self.matrix_low_0 = torch.tensor(matrix_h_0, dtype=torch.float32, device='cuda')
            self.matrix_low_1 = torch.tensor(matrix_h_1, dtype=torch.float32, device='cuda')
            self.matrix_high_0 = torch.tensor(matrix_g_0, dtype=torch.float32, device='cuda')
            self.matrix_high_1 = torch.tensor(matrix_g_1, dtype=torch.float32, device='cuda')
        else:
            self.matrix_low_0 = torch.tensor(matrix_h_0, dtype=torch.float32)
            self.matrix_low_1 = torch.tensor(matrix_h_1, dtype=torch.float32)
            self.matrix_high_0 = torch.tensor(matrix_g_0, dtype=torch.float32)
            self.matrix_high_1 = torch.tensor(matrix_g_1, dtype=torch.float32)
